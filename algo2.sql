...
BEGIN
	...
	for tupla in select distinct cid_ciudad_origen, cid_ciudad_destino from destinos where cid_ciudad_origen=origen loop
		a:= tupla.cid_ciudad_origen;
		b:= tupla.cid_ciudad_destino;
		insert into tabla values(concat(concat(a,','),b));
	end loop;
	for tupla in select distinct foo.oid, foo.did, destinos.cid_ciudad_destino from (select cid_ciudad_origen, cid_ciudad_destino from destinos where cid_ciudad_origen=origen) as foo(oid,did) inner join destinos on foo.did=destinos.cid_ciudad_origen loop
		a:= tupla.oid;
		b:= tupla.did;
		c:= tupla.cid_ciudad_destino;
		insert into tabla values(concat(concat(concat(concat(a,','),b),','),c));
	end loop;
	for tupla in select distinct id1,id2,id3, cid_ciudad_destino from (select distinct foo.oid, foo.did, destinos.cid_ciudad_destino from (select cid_ciudad_origen, cid_ciudad_destino from destinos where cid_ciudad_origen=origen) as foo(oid,did) inner join destinos on foo.did=destinos.cid_ciudad_origen) as vee(id1,id2,id3) inner join destinos on vee.id3=destinos.cid_ciudad_origen loop
		a:= tupla.id1;
		b:= tupla.id2;
		c:= tupla.id3;
